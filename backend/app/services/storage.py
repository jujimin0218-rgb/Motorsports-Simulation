...
class SaveStore:
    ...
    def save(
        ...
        *,
        ...
        identifier = existing["id"] if existing else (save_id or uuid.uuid4().hex)
        ...
        summary = SaveSummary(
            ...
            created_at=created,
            updated_at=now,
            slot=slot if slot is not None else (existing["slot"] if existing else None),
        )
        self._connection.execute(
            ...
            "player_team=excluded.player_team, updated_at=excluded.updated_at, "
    ...
    def delete(self, save_id: str) -> None:
        cursor = self._connection.execute("DELETE FROM saves WHERE id = ?", (save_id,))
        self._connection.commit()
